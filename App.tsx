    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col">
      ...
      <nav className="bg-white border-b border-slate-200 px-4 py-3 sticky top-0 z-30">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold tracking-tight hidden sm:block">Nihongo Flashcards</h1>
          </div>
          <div className="flex items-center gap-2">
             <button
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 w-full max-w-4xl mx-auto">
        ...
        <div className="w-full max-w-md mb-6 flex justify-between items-end px-2">
            <div>
                <span className="text-3xl font-bold text-indigo-600">{currentIndex + 1}</span>
                <span className="text-slate-400 text-sm font-medium"> / {deck.length}</span>
            </div>
            <div className="flex items-center gap-2">
                <button 
                    onClick={handleShuffleToggle}
                    className={`p-2 rounded-full transition-colors ${randomMode ? 'bg-indigo-100 text-indigo-600' : 'text-slate-400 hover:bg-slate-100'}`}
                    title="Xáo trộn"
                >
                    <Shuffle size={20} />
                </button>
